...
- **An expiry** — every entity is time-scoped: you set how long it lives when you create it (the SDK's `expiresIn`, in seconds), and you can extend it later. You pay by bytes × lifetime, not forever.
...
- **Numeric attributes** support range queries (`gt`, `lt`, `gte`, `lte`). Use for anything you'll filter by range — timestamps, scores, counts, prices.
...
### Challenge 2 · Marketplaces — Aug 12 – 20 · host: Shantelle Awomoyi
...
### Challenge 3 · DeFi — Aug 21 – 31 · host: Marcos Miranda
